...
class TestEncoderHandler(IsolatedAsyncioTestCase):
    ...
    @patch('core.EncoderHandler.image_hasher')
    async def test_encode_with_cached_image(self, mock_image_hasher,
                                            mock_image_encoder_proxy,
                                            mock_db_proxy):
        mock_image_hasher.hash.return_value = self.MOCK_IMAGE_HASH
        ...
        mock_db_proxy.update_session.assert_called_once_with(self.MOCK_SESSION_ID, self.MOCK_IMAGE_HASH)

    @patch('core.EncoderHandler.db_proxy')
    @patch('core.EncoderHandler.image_encoder_proxy')
    @patch('core.EncoderHandler.image_hasher')
    async def test_encode_with_not_cached_image(self, mock_image_hasher,
                                                mock_image_encoder_proxy,
                                                mock_db_proxy):
        mock_image_hasher.hash.return_value = self.MOCK_IMAGE_HASH
